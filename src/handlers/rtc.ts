...
function _rtc(channel_id: string) {
  return `${channel_id}.rtc`;
}
...
export function addRtcHandlers(client: Client) {
  ...
  client.socket.on(
    ...
    wrapper.event(
      async (channel_id) => {
        ...
        // Leave prev room
        if (client.current_room) client.socket.leave(_rtc(client.current_room));
  ...
  client.socket.on(
    'rtc:left',
    wrapper.event(async (channel_id) => {
      ...
      io()
        .to(_rtc(channel_id))
        .to(channel.domain)
        .emit('rtc:user-left', channel.domain, channel_id, client.profile_id);
    }),
  );
}
