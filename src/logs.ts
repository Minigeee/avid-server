...
////////////////////////////////////////////////////////////
const LOG_LOCAL =
  config.logger.mode === 'full' || config.logger.mode === 'local';
const LOG_REMOTE =
  config.logger.mode === 'full' || config.logger.mode === 'remote';

////////////////////////////////////////////////////////////
const LEVEL_NAMES = ['error', 'warn', 'info', 'verbose', 'debug'];
...
////////////////////////////////////////////////////////////
const LEVEL_COLORS: string[] = ['red', 'yellow', 'green', 'cyan', 'bmagenta'];
  ...
  log(message: string, options: LogOptions = {}): void {
    ...
    if (LOG_LOCAL && (options.console || options.console === undefined)) {
      ...
      const colors = {
        ...
        method: this.method
          ? CONSOLE_COLORS[METHOD_COLORS[this.method]]
      ...
      console.log(
        printf(
          `${colors.time}[%s]${colors.reset} | ${colors.method}%-6s${colors.reset} | %-35s | ${colors.level}%-12s %s`,
          timeStr,
    ...
    if (config.logger.log_file && !debug) {
      const fname = config.dev_mode
        ? 'server.log'
        : `logs/${new Date().toISOString().slice(0, 10)}.${process.pid}.log`;
      appendFile(
        fname,
        JSON.stringify(denullify({ ...log, location: null })) + ',\n',
        (err) => {
          if (err) {
            this._logRemote({
              ...log,
