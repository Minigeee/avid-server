...
const routes: ApiRoutes<`${string} /channel_groups${string}`> = {
	"GET /channel_groups": {
		...
		code: async (req, res) => {
			const results = await query<ChannelGroup[]>(sql.multi([
	...
	"POST /channel_groups": {
		...
		permissions: (req) => sql.return(hasPermission(req.token.profile_id, req.body.domain, 'can_create_groups', req.body.domain)),
		code: async (req, res) => {
			// List of operations
			const ops = [
			...
			// Create group
			const results = await query<ChannelGroup>(sql.transaction(ops), { log: req.log });
			assert(results);
	...
	"PATCH /channel_groups/:group_id": {
		validate: {
			...
			after: {
				required: false,
				location: 'body',
				transform: (value) => value === null ? null : isRecord(value, 'channel_groups'),
			},
		},
		permissions: (req) => {
			const ops = [];
			...
			if (req.body.name !== undefined)
				ops.push(hasPermission(req.token.profile_id, req.params.group_id, 'can_manage'));
			if (req.body.after !== undefined)
				ops.push(hasPermission(req.token.profile_id, `${req.params.group_id}.domain`, 'can_manage', `${req.params.group_id}.domain`));
		...
		code: async (req, res) => {
			...
			if (req.body.after !== undefined) {
				...
				ops.push(sql.update<Domain>(`(${req.params.group_id}.domain)`, {
					set: {
						groups: sql.fn<Domain>(function () {
			...
			if (groupUpdate)
				ops.push(sql.update<ChannelGroup>(req.params.group_id, { set: { name: req.body.name } }));
	...
	"DELETE /channel_groups/:group_id": {
		...
		permissions: (req) => sql.return(hasPermission(req.token.profile_id, req.params.group_id, 'can_delete_group')),
		code: async (req, res) => {
