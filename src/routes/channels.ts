...
const routes: ApiRoutes<`${string} /channels${string}`> = {
  ...
  'GET /channels/:channel_id': {
    ...
    permissions: (req) =>
      sql.return(
        hasPermission(req.token.profile_id, req.params.channel_id, 'can_view'),
      ),
    code: async (req, res) => {
      const results = await query<Channel[]>(
  ...
  'PATCH /channels/:channel_id': {
    validate: {
      ...
      after: {
        ...
        location: 'body',
        transform: (value) =>
          value === null ? null : isRecord(value, 'channels'),
      },
    },
    permissions: (req) => {
      const ops: string[] = [
        sql.let(
          '$member',
          getMember(req.token.profile_id, `${req.params.channel_id}.domain`),
        ),
      ];
      ...
      if (req.body.after !== undefined) {
        ...
        ops.push(
          sql.let(
            '$old_group',
            `${req.params.channel_id}.inherit || ${sql.wrap(
              sql.select<ChannelGroup>(['VALUE id'], {
                from: 'channel_groups',
                where: sql.match<ChannelGroup>({
                  channels: ['CONTAINS', req.params.channel_id],
                }),
              }),
              { append: '[0]' },
            )}`,
      ...
      return sql.multi([...ops, sql.return(conds.join('&&'))]);
    },
    code: async (req, res) => {
      const ops: string[] = [];
      ...
      if (req.body.after !== undefined) {
        ...
        if (same) {
          ops.push(
            sql.update<ChannelGroup>('($old_group)', {
              set: {
                channels: sql.fn<ChannelGroup>(
                  function () {
                    ...
                    if (from >= 0) this.channels.splice(from, 1);
                    this.channels.splice(
                      to,
                      0,
                      new_Record('channels', target_id),
                    );
        ...
        // Handle move to new group
          ...
          // Modify dst group
          ops.push(
            sql.update<ChannelGroup>(req.body.group, {
              set: {
                channels: sql.fn<ChannelGroup>(
                  function () {
                    const to = before
          ...
          ops.push(
            sql.let(
              ...
              sql.if(
                {
                  cond: `${req.params.channel_id}.inherit == NONE`,
                  body: 'NONE',
                },
                { body: req.body.group },
              ),
