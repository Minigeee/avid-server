    ...
    code: async (req, res) => {
      ...
      >(
        sql.multi([
          sql.let(
            '$member',
            getMember(req.token.profile_id, req.params.domain_id),
          ),
      ...
      // Member info
      const info: Omit<UserPermissions, 'permissions'> & {
        permissions: Record<string, Set<string> | string[]>;
      } = {
        roles: member.roles || [],
      ...
      for (const entry of entries) {
        if (!info.permissions[entry.resource])
          info.permissions[entry.resource] = new Set<AllPermissions>(
            entry.permissions,
          );
        else {
          const set = info.permissions[entry.resource] as Set<string>;
          for (const perm of entry.permissions) set.add(perm);
        }
      }
      ...
      for (const group of groups) groupMap[group.id] = group;
  ...
  'PUT /domains/:domain_id/role_order': {
    validate: {
      ...
      roles: {
        ...
        location: 'body',
        transform: (value) =>
          isArray(value, (value) => isRecord(value, 'roles')),
      },
    },
    permissions: (req) =>
      sql.return(
  ...
  'GET /domains/join/:join_id': {
    ...
    code: async (req, res) => {
      ...
      const results = await query<(Domain & { members: string[] })[]>(
        sql.select<Domain>(
          [
            'name',
            'icon',
            sql.wrap(`<-(member_of WHERE in = ${req.token.profile_id})`, {
              alias: 'members',
            }),
          ],
          {
      ...
      return {
        name: results[0].name,
        icon: results[0].icon,
        is_member: results[0].members.length > 0,
      };
    },
  },
