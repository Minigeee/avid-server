      ...
      if (req.query.ids) {
        ...
        if (req.query.search)
          matchConstraints.push(
            `string::lowercase(alias) CONTAINS '${req.query.search}'`,
          );
        if (req.query.role)
        ...
        if (req.query.online !== undefined)
          matchConstraints.push(
            `in.online${req.query.online ? '=true' : '!=true'}`,
          );
        const matchStr = matchConstraints.join('&&');

        // Limit
        const limit = Math.min(
          req.query.limit || config.db.page_size.members,
          1000,
        );
        ...
        // Create query string
        const results = await query<any[]>(sql.multi(ops), {
          complete: true,
          log: req.log,
        });
        ...
        return {
          ...
          count:
            results[results.length - 1].length > 0
              ? results[results.length - 1][0].count
              : 0,
        };
      }
    },
    ...
    permissions: (req) =>
      sql.return(
        `${hasMemberPermission(
          ...
          'can_assign_role',
          req.query.domain,
        )} FROM [${req.body.roles.map((x) => `{value: ${x}}`)}]))`,
      ),
    code: async (req, res) => {
