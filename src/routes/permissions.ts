  ...
  'GET /permissions': {
    ...
    permissions: (req) => {
      if (req.query.domain)
        ...
        return sql.return(
          isMember(req.token.profile_id, `${req.query.role}.domain`),
        );
      else if (req.query.resource)
        return sql.return(
          isMember(
            req.token.profile_id,
            req.query.resource_type === 'domains'
              ? req.query.resource
              : `${req.query.resource}.domain`,
          ),
        );
      else
        throw new StatusError(
          'at least one of "query.domain", "query.resource", or "query.role" must be provided',
          { status: 400 },
        );
    },
    code: async (req, res) => {
      // Match conditions
      const match: any = {};
      if (req.query.domain) match.domain = req.query.domain;
      if (req.query.resource) match.resource = req.query.resource;
      if (req.query.role) match.role = req.query.role;

      // Domain
      const domain =
        req.query.domain ||
        (req.query.resource && req.query.resource_type === 'domains')
          ? req.query.resource
          : `${req.query.role || req.query.resource}.domain`;
      ...
      const results = await query<AclEntry[]>(
        sql.multi([
          ...
          sql.select<AclEntry>(['domain', 'resource', 'role', 'permissions'], {
            from: 'acl',
  ...
  'PATCH /permissions': {
        ...
        transform: (value) =>
          isArray(value, (value) => {
            if (
              ...
              throw new Error(
                'must be a permissions object with a string `resource` and `role` field',
              );
    ...
    permissions: (req) =>
      sql.multi([
        ...
        sql.return(
          `array::all((SELECT VALUE ${canModifyAcl(
            'resource',
            'role',
            'permissions',
          )} FROM [${req.body.permissions
            .map(
              (x) =>
                `{resource: ${x.resource}, role: ${
                  x.role
    ...
    code: async (req, res) => {
      ...
      const deleted = req.body.permissions
        .filter((x) => x.permissions.length === 0)
        .map((x) => ({ ...x, domain: req.body.domain }));
      ...
      if (updateIndices.length > 0) {
        ops.splice(
          ...
          sql.let(
            '$entries',
            sql.select<AclEntry>(
              ['VALUE string::concat(resource, ".", role)'],
              {
                from: 'acl',
                where: updateIndices
                  .map((i) => {
      ...
      // Perform transaction
      const results = await query<(AclEntry | AclEntry[])[]>(
        sql.transaction(ops),
        ...
        updated: updateIndices
          .map((i) =>
            Array.isArray(results[i])
              ? (results[i] as AclEntry[])[0]
              : (results[i] as AclEntry),
          )
