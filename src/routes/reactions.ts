...
const routes: ApiRoutes<`${string} /reactions${string}`> = {
  'POST /reactions': {
    ...
    code: async (req, res) => {
      // Create reaction
      const channel = await query<string>(
        sql.multi([
          sql.relate<{ emoji: string }>(
            req.token.profile_id,
            'reactions',
            req.body.message,
            {
  ...
  'DELETE /reactions': {
    ...
    code: async (req, res) => {
      ...
      if (req.query.member) conds['in'] = req.query.member;
      if (req.query.emoji) conds['emoji'] = req.query.emoji;

      // Create reaction
      const results = await query<[Reaction[], string]>(
        sql.multi([
      ...
      if (req.query.emoji)
        ...
        await getChannel(results[1]).then((channel) => {
          io()
            .to(channel.domain)
            .emit('chat:reactions', results[1], req.query.message, {}, true);
        });
      }
    },
  },
