...
const routes: ApiRoutes<`${string} /roles${string}`> = {
  'GET /roles': {
    ...
    code: async (req, res) => {
      const results = await query<Role[]>(
        sql.select<Role>('*', {
          from: 'roles',
          where: sql.match<Role>({ domain: req.query.domain }),
        }),
        { log: req.log },
      );
      assert(results);
      ...
      roles: {
        ...
        transform: (value) => {
          if (!Array.isArray(value))
            throw new Error(
              'must be an array of role objects to update where each object must contain an `id` value',
            );
          ...
          for (let i = 0; i < value.length; ++i) {
            ...
            if (typeof role !== 'object' || !role.id?.startsWith('roles:'))
              throw new Error(
                `[${i}] must be a role object with an \`id\` property in the form "roles:[id]"`,
              );
          }
    ...
    permissions: (req) =>
      sql.return(
        `array::all((SELECT VALUE ${hasPermission(
          ...
          'value',
          'can_manage',
        )} FROM [${req.body.roles.map((x) => `{value: ${x.id}}`).join(',')}]))`,
      ),
    code: async (req, res) => {
  ...
  'PATCH /roles/:role_id': {
    validate: {
      ...
      after: {
        ...
        location: 'body',
        transform: (value) =>
          value === null ? null : asRecord('roles', value),
      },
    },
    permissions: (req) => {
      const ops = [];
      ...
      if (
        ...
        req.body.show_badge !== undefined
      )
        ops.push(
          hasPermission(req.token.profile_id, req.params.role_id, 'can_manage'),
        );
    ...
    code: async (req, res) => {
            ...
            set: {
              roles: sql.fn<Domain>(
                function () {
                  ...
                  const from = this.roles.findIndex(
                    (x) => x.toString() === targetRecord,
                  );
                  const to = before
                    ? this.roles.findIndex((x) => x.toString() === before) + 1
                    : 0;

                  if (from >= 0) this.roles.splice(from, 1);
      ...
      if (
        ...
        ops.push(
          sql.update<Role>(req.params.role_id, {
            set: {
      ...
      const results = await query<(Role[] | { roles: string[] }[])[]>(
        sql.transaction(ops),
        { complete: true, log: req.log },
      );
      assert(results && results.length > 0);
      ...
      return {
        role: (reorder
          ...
          : results[0][0]) as Role,
        order: reorder
          ? (results[0] as { roles: string[] }[])[0].roles
          : undefined,
      };
    },
  },
  ...
  'DELETE /roles/:role_id': {
    ...
    code: async (req, res) => {
      await query<Role[]>(sql.delete<Role>(req.params.role_id), {
        log: req.log,
      });
  ...
  'PATCH /roles/:role_id/members': {
    validate: {
      ...
      members: {
        ...
        location: 'body',
        transform: (value) =>
          isArray(value, (value) => isRecord(value, 'profiles')),
      },
    },
    permissions: (req) =>
      sql.return(
        `${hasPermission(
          ...
          req.params.role_id,
          'can_assign_role',
        )} || array::all((SELECT VALUE ${hasMemberPermission(
          req.token.profile_id,
          'value',
          'can_manage_member_roles',
          `${req.params.role_id}.domain`,
        )} FROM [${req.body.members.map((x) => `{value: ${x}}`)}]))`,
      ),
    code: async (req, res) => {
