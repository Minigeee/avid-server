...
const routes: ApiRoutes<`${string} /tasks${string}`> = {
  'GET /tasks': {
    ...
    permissions: (req) =>
      sql.return(
        hasPermission(req.token.profile_id, req.query.board, 'can_view'),
      ),
    code: async (req, res) => {
      // Get tasks and assignees
      const results = await query<[unknown, unknown, ExpandedMember[], Task[]]>(
        sql.multi([
          // Get tasks
          sql.let(
            '$tasks',
  ...
  'POST /tasks': {
    validate: {
      ...
      dependencies: {
        ...
        location: 'body',
        transform: (value) =>
          isArray(value, (value) => isRecord(value, 'tasks')),
      },
      description: {
        required: false,
        location: 'body',
    ...
    code: async (req, res) => {
      ...
      const results = await query<[string, Task[]]>(
        sql.transaction([
          // Increment counter
          sql.update<Board>(req.body.board, {
            set: { _task_counter: ['+=', 1] },
            return: 'NONE',
          }),
  ...
  'PATCH /tasks': {
        ...
        transform: (value) =>
          isArray(value, (value) => {
            ...
            if (
              ...
              !value.id.startsWith('tasks:')
            )
              throw new Error(
                'must have an "id" field in the form "tasks:[id]"',
              );
    ...
    permissions: (req) => {
      ...
      if (req.body.update)
        conds.push(
          `array::all((SELECT VALUE (${hasPermission(
            req.token.profile_id,
            'task.board',
            'can_manage_tasks',
          )} || (task.assignee == ${
            ...
            'task.board',
            'can_manage_own_tasks',
          )})) FROM [${req.body.update
            .map(
              (task) =>
                `{task: ${task.id}, assignee: ${
                  task.assignee === undefined ? 'NONE' : task.assignee
                }}`,
            )
            .join(',')}]))`,
        );
      ...
      if (req.body.delete)
        conds.push(
          `array::all((SELECT VALUE (${hasPermission(
            ...
            'task.board',
            'can_manage_tasks',
          )} || (task.assignee == ${req.token.profile_id} && ${hasPermission(
            req.token.profile_id,
            'task.board',
            'can_manage_own_tasks',
          )})) FROM [${req.body.delete
            .map((id) => `{task: ${id}}`)
            .join(',')}]))`,
        );
    ...
    code: async (req, res) => {
      ...
      if (req.body.update?.length || req.body.delete?.length)
        ops.push(
          sql.return(
            `${
              req.body.update?.length
                ? req.body.update[0].id
                : req.body.delete?.[0]
            }.board.channel`,
          ),
        );
          ...
          ...req.body.update.map((raw) => {
            ...
            return sql.update<Task>(raw.id, {
              set: {
                ...
                time_updated: now,
                time_status_updated:
                  task.status !== undefined ? now : undefined,
              },
            });
          }),
        );
      ...
      // Delete
      if (req.body.delete) {
        ops.push(sql.delete(req.body.delete));
      }
      ...
      // Execute ops
      const results = await query<Task[][]>(sql.transaction(ops), {
        complete: true,
        log: req.log,
      });
      assert(
        results &&
          results.length ===
            (req.body.update?.length || 0) + (req.body.delete ? 1 : 0) + 1,
      );
      ...
      return {
        updated: results.slice(1, req.body.update.length + 1).map((x) => x[0]),
      };
    },
  },
  ...
  'GET /tasks/:task_id': {
    ...
    permissions: (req) =>
      sql.return(
        hasPermission(
          req.token.profile_id,
          `${req.params.task_id}.board`,
          'can_view',
        ),
      ),
    code: async (req, res) => {
      const results = await query<
        [unknown, ExpandedMember[], Task & { _domain?: string }]
      >(
        sql.multi([
          sql.let(
            '$task',
      ...
      return {
        ...results[2],
        assignee:
          results[2].assignee && results[1].length > 0
            ? (omitBy(results[1][0], isNil) as ExpandedMember)
            : undefined,
      };
    },
  },
    ...
    permissions: (req) =>
      sql.return(
        `${hasPermission(
          ...
          `${req.params.task_id}.board`,
          'can_manage_tasks',
        )} || (${req.params.task_id}.assignee == ${req.token.profile_id} ${
          req.body.assignee !== undefined
            ? `&& ${req.body.assignee} == ${req.token.profile_id} `
            : ''
        }&& ${hasPermission(
          req.token.profile_id,
          `${req.params.task_id}.board`,
  ...
  'DELETE /tasks/:task_id': {
    ...
    code: async (req, res) => {
      const results = await query<[string, unknown]>(
        sql.multi([
          sql.return(`${req.params.task_id}.board.channel`),
