...
type Batcher<E extends keyof BatchEvents> = {
  ...
  create: (
    batch: Partial<BatchData[E]>,
    ...args: Parameters<BatchEvents[E]>
  ) => void;
...
const _batchers: { [E in keyof BatchEvents]: Batcher<E> } = {
  'chat:reactions': {
    ...
    add(batch, channel_id, message_id, emoji, delta) {
      if (!batch.changes?.[emoji]) batch.changes[emoji] = delta;
      else batch.changes[emoji] += delta;
    },
 ...
 * @param args
 */
export function emitBatchEvent<E extends keyof BatchEvents>(
  event: E,
  key: string,
  ...args: Parameters<BatchEvents[E]>
) {
