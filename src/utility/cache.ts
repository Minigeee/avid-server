...
export class SyncCache<T> {
	 ...
	 */
	get<K extends string | string[]>(key: K): K extends ReadonlyArray<infer A> ? (T | null)[] : (T | null) {
		let keys: string[];
		if (!Array.isArray(key))
			keys = [key];
		else
		...
		for (let i = 0; i < keys.length; ++i) {
			...
			if (!cached || elapsed > this.invalidate_time)
				objects.push(null);
			else
				// Add data to list
				objects.push(cached.data);
		}

		// Return data
		if (!Array.isArray(key))
			return objects[0] as K extends ReadonlyArray<infer A> ? (T | null)[] : (T | null);
		else
			return objects as K extends ReadonlyArray<infer A> ? (T | null)[] : (T | null);
	}
...
export class AsyncCache<T> {
	...
	////////////////////////////////////////////////////////////
	constructor(fetch: (keys: string[]) => Promise<T[]>, invalidate_time: number = config.db.cache_lifetime) {
		this.fetch = fetch;
		this.invalidate_time = invalidate_time * 1000;
