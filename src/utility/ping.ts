...
export async function ping(
  ...
  const member_ids = options.member_ids
    ? Array.from(new Set(options.member_ids))
    : [];
  ...
  const ops = [
    ...
    sql.let(
      '$app_states',
      sql.fn(function (members: string[]) {
        return members.map((x) =>
          new_Record('app_states', x.toString().split(':')[1]),
        );
      }).__esc__,
    ),
    ...
    ops.splice(
      ...
      sql.let(
        ...
        sql.select<Member>(['in'], {
          ...
          where: sql.match<Member>({
            roles: ['CONTAINSANY', options.role_ids],
          }),
          value: true,
        }),
      ),
  ...
  for (const profile_id of profile_ids) {
    // Skip sender
    if (profile_id === options.sender_id) continue;
