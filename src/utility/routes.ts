...
export type ApiRequest<Path extends ApiPath> = Request<
	ApiSchema[Path] extends { params: string[] } ? Record<ApiSchema[Path]['params'][number], string> : {},
	any,
...
export type ApiRequestHandler<Path extends ApiPath> = (req: ApiRequest<Path>, res: Response) => Promise<void> | void;

/** Helper function for transform map */
type _Validate<T, Loc extends string, Path extends ApiPath> = { [K in keyof T]-?: {
...
export type ApiRouteDefinition<Path extends ApiPath = ApiPath> = {
	...
	validate:
	(ApiSchema[Path] extends { params: string[] } ? _Validate<Record<ApiSchema[Path]['params'][number], string>, 'params', Path> : {}) &
	(ApiSchema[Path] extends { query: any } ? _Validate<ApiSchema[Path]['query'], 'query', Path> : {}) &
	(ApiSchema[Path] extends { body: any } ? _Validate<ApiSchema[Path]['body'], 'body', Path> : {});
	...
	code: (req: ApiRequest<Path>, res: Response) => Promise<ApiSchema[Path] extends { return: any } ? ApiSchema[Path]['return'] : void>;
	/** Extra middlewares */
	middleware?: {
...
function createHandlers(def: ApiRouteDefinition<'GET /messages'>) {
	...
	// Middlewares
	const preTransforms = def.middleware?.filter(x => x.before === 'transforms');
	const prePermissions = def.middleware?.filter(x => x.before === 'permissions');
	...
	handlers.push(
		(req, res, next) => {
			...
			for (const [name, opts] of Object.entries(def.validate)) {
				...
				try {
					...
					if (newData !== undefined)
						req[opts.location][name] = newData as any;
				}
				catch (err: any) {
			...
			if (errors.length > 0) {
				throw new StatusError(errors.map(x => `"${x.opts.location}.${x.name}" ${x.msg}`).join(' | '), { status: 400 });
			}
			else
				next();
		}
	...
	if (def.permissions) {
		handlers.push(
			wrapper.api(async (req, res) => {
				...
				const hasPermission = await query<boolean>(def.permissions(req as any), { log: req.log });
				if (!hasPermission)
					throw new StatusError('you do not have permission to use this route', { status: 403 });
			})
		);
	}
	...
	// End
	handlers.push(wrapper.api(async (req, res) => {
...
export function createRoutes(app: Express, routes: ApiRoutes<ApiPath>) {
	...
	for (const [route, def] of Object.entries(routes)) {
		...
		assert(method === 'get' || method === 'post' || method === 'put' || method === 'patch' || method === 'delete');
	...
	for (const [routerName, paths] of Object.entries(grouped)) {
		...
		for (const [path, methods] of Object.entries(paths)) {
			...
			if (methods.get)
				route.get(createHandlers(methods.get as any));
			if (methods.post)
				route.post(createHandlers(methods.post as any));
			if (methods.put)
				route.put(createHandlers(methods.put as any));
			if (methods.patch)
				route.patch(createHandlers(methods.patch as any));
			if (methods.delete)
				route.delete(createHandlers(methods.delete as any));
		}
