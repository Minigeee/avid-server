...
export function isObject<T>(
  ...
  if (typeof value !== 'object')
    throw new Error(
      `must be an object with fields ${entries
...
export function isArray<T>(
  ...
  if (
    !Array.isArray(value) ||
    (options?.minlen !== undefined && value.length < options.minlen) ||
    (options?.maxlen !== undefined && value.length > options.maxlen)
  ) {
    const hasMin = options?.minlen !== undefined;
    const hasMax = options?.maxlen !== undefined;
    throw new Error(
      `must be an array ${hasMin || hasMax ? 'with a length ' : ''}${
        hasMin ? `>=${options.minlen}${hasMax ? ' and ' : ''}` : ''
      }${hasMax ? `<=${options.maxlen}` : ''}`,
    );
  }
...
export function isIn<T>(value: any, values: T[]) {
  if (values.indexOf(value) < 0)
    throw new Error(
      `must be one of ${values.map((x) => `"${x}"`).join(' | ')}`,
    );
  return value;
}
...
export function asRecord(table: string, str: string) {
  if (!/^\w+$/.test(str))
    throw new Error(
      `must be a valid alphanumeric id that is not in the form "${table}:[id]"`,
    );
